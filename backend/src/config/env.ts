...
const envSchema = z
  .object({
    ...
    TOTP_ISSUER: z.string().min(1).max(64).default('MCPanel'),
    ...
    MC_HEAP_RATIO: z.coerce.number().min(0.5).max(0.9).default(0.75),
    ...
    MODRINTH_USER_AGENT: z.string().min(3).default('minecraft-panel/0.1 (self-hosted)'),
  ...
  .superRefine((env, ctx) => {
    ...
    if (env.NODE_ENV === 'production' && env.MC_BIND_ADDRESS === '127.0.0.1') {
      ctx.addIssue({
        code: 'custom',
        path: ['MC_BIND_ADDRESS'],
        message: 'MC_BIND_ADDRESS is loopback-only; players will not be able to connect remotely',
      });
    }
  });
