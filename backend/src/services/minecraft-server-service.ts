...
export class MinecraftServerService {
  ...
  async create(input: CreateMinecraftServerInput): Promise<ServerDto> {
    ...
    try {
      ...
      const created = await this.runtime.create({
        ...
        javaVersion: server.javaVersion as JavaVersion,
        restartPolicy: server.restartPolicy as RestartPolicy,
        port: server.port,
        maxPlayers: server.maxPlayers,
        jvmFlags: server.jvmFlags,
      });
      const updated = await this.servers.update(server.id, {
  ...
  private async updateUnlocked(id: string, input: UpdateMinecraftServerInput): Promise<ServerDto> {
    ...
    if (input.port !== undefined && input.port !== current.port) {
      const portOwner = await this.servers.findByPort(input.port);
      if (portOwner && portOwner.id !== id) {
        throw new ConflictError('That host port is already assigned to another server');
      }
    }
  ...
  async delete(id: string): Promise<void> {
    await this.lock.run(id, async () => {
      ...
      if (backupPath.startsWith(`${backupBase}${sep}`)) {
  ...
  async start(id: string): Promise<ServerDto> {
    return this.runAction(id, 'starting', async (server) => this.runtime.start(server), 'online');
  }

  async stop(id: string): Promise<ServerDto> {
    return this.runAction(id, 'stopping', async (server) => this.runtime.stop(server), 'offline');
  }

  async restart(id: string): Promise<ServerDto> {
    return this.runAction(id, 'starting', async (server) => this.runtime.restart(server), 'online');
  }

  async kill(id: string): Promise<ServerDto> {
    return this.runAction(id, 'stopping', async (server) => this.runtime.kill(server), 'offline');
  }

  async logs(id: string, tail: number): Promise<string[]> {
    const server = await this.servers.requireById(id);
    return this.runtime.logs(server, tail);
  }

  async command(id: string, command: string): Promise<string> {
    const server = await this.servers.requireById(id);
    if ((await this.runtime.status(server)) !== 'running') {
      throw new ConflictError('Server must be online to send console commands');
    }
  ...
  async streamLogs(id: string) {
    const server = await this.servers.requireById(id);
    return this.runtime.streamLogs(server);
  }

  private async runAction(
    id: string,
    transition: string,
    action: (server: ServerRecord) => Promise<void>,
    success: string,
  ): Promise<ServerDto> {
  ...
  private async recreateContainer(updated: ServerRecord, previous: ServerRecord): Promise<void> {
    const wasRunning =
      (await this.runtime.status(previous).catch(() => 'missing' as const)) === 'running';
    let previousRemoved = false;
    let replacementContainerId: string | null = null;

    try {
      if (previous.containerId) {
        await this.runtime.remove(previous);
        previousRemoved = true;
      }

      const created = await this.runtime.create(this.toContainerSpec(updated));
      replacementContainerId = created.id;
      const record = await this.servers.update(updated.id, {
        containerId: created.id,
        status: 'offline',
      });
      if (wasRunning) {
        await this.runtime.start(record);
        await this.servers.update(updated.id, { status: 'online' });
      }
    } catch (error) {
      if (replacementContainerId) {
        await this.runtime
          .remove({ ...updated, containerId: replacementContainerId })
          .catch(() => undefined);
      }
      ...
      try {
        ...
        if (previousRemoved) {
          const restored = await this.runtime.create(this.toContainerSpec(previous));
          rollbackContainerId = restored.id;
        }

        const restoredRecord = await this.servers.update(updated.id, {
          ...runtimeRollbackFields(previous),
          containerId: rollbackContainerId,
          status: 'offline',
        });
        await this.patchProperty(restoredRecord, 'max-players', String(previous.maxPlayers)).catch(
          () => undefined,
        );

        if (wasRunning && restoredRecord.containerId) {
          await this.runtime.start(restoredRecord);
          await this.servers.update(updated.id, { status: 'online' });
        }
      } catch {
        await this.servers
          .update(updated.id, { containerId: null, status: 'error' })
          .catch(() => undefined);
        throw new AppError(
          'Server configuration failed and the previous container could not be restored',
          502,
          'SERVER_ROLLBACK_FAILED',
        );
      }
  ...
  private toDto(server: ServerRecord): ServerDto {
    return {
      ...
      port: server.port,
      maxPlayers: server.maxPlayers,
      address: `${server.customDomain ?? this.env.PUBLIC_HOST}:${server.port}`,
...
function runtimeRollbackFields(previous: ServerRecord): UpdateServerRecord {
  return {
    ...
    jvmFlags: previous.jvmFlags,
    autoRestartSchedule: previous.autoRestartSchedule,
